...
class PricePredictor:
    ...
    def buildTrainDataSet(self):
        ...
        self.mean_price = np.mean(data);
        self.std_price = np.std(data)
        self.normalize_data = (data - self.mean_price) / self.std_price #标准化

        self.normalize_data = self.normalize_data[:,np.newaxis]  #增加维度
        for i in range(len(self.normalize_data)-self.timeStep-1):
            x=self.normalize_data[i:i+self.timeStep]
            y=self.normalize_data[i+1:i+self.timeStep+1]
    ...
    def prediction(self):
        ...
        with tf.Session() as sess:
            ...
            # 以折线图表示结果
            plt.figure()
            true_price = self.std_price**predict
            true_price = [price + self.mean_price for price in true_price]
            plt.plot(list(range(len(self.normalize_data), len(self.normalize_data) + len(predict))), true_price, color='r')
            plt.show()
